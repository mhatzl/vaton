...
package body Long_Integers is
   ...
   procedure Create_Positive_Integer(T : in out Test_Fixture) is
      ...
      Success : Boolean;
      Result : Vaton.Number;
      Expected : Standard.Long_Integer := 0;
   begin
      while Digit_Array.Length (Partial_Number.Whole) * BASE_10_LENGTH_TO_BIT_SIZE < Standard.Integer'Size loop
         Vaton.Append(Partial_Number, '1', Success);
         Assert(Success, "Appending number failed");
         
         Expected := (Expected + 1) * Standard.Long_Integer(BASE_10);
      end loop;
      Expected := Expected / Standard.Long_Integer(BASE_10);
      
      Result := Vaton.To_Number(Partial_Number);
      Assert(Result.Kind = Vaton.Long_Integer, "Result not of type Standard.Long_Integer");
      Assert(Result.Long_Integer = Expected, "Conversion resulted in wrong integer number");
