package body Vaton is
         ...
         if not Digit_Array.Is_Empty(Partial_Number.Exponent) then
            ...
            begin
               ...
               if Success then
                  ...
                  if Success and then To_Integer(Split_Exponent, Partial_Number.Exponent_Is_Negative).Kind /= Integer then
                     return False;
                  end if;
               end if;
            end;
   ...
   begin
      if Digit_Array.Is_Empty(Partial_Number.Whole) and then
        (not Digit_Array.Is_Empty(Partial_Number.Fraction) or else not Digit_Array.Is_Empty(Partial_Number.Exponent)
         ...
         return False;
      elsif not Digit_Array.Is_Empty(Partial_Number.Exponent) and then To_Integer(Partial_Number.Exponent, Partial_Number.Exponent_Is_Negative).Kind /= Integer then
         return False;
      else
         return True;
      end if;
   ...
   begin
      if Digit_Array.Is_Empty(Partial_Number.Whole) or else Digit_Array.Length(Partial_Number.Whole) > BIG_NUMBER_DIGIT_LIMIT - Digit_Array.Length(Partial_Number.Fraction) then
         ...
         begin
            if Digit_Array.Length(Partial_Number.Whole) + Digit_Array.Length(Partial_Number.Fraction) < Standard.Float'Digits
              ...
              ((Partial_Number.Exponent_Is_Negative and then Exponent.Integer > MIN_FLOAT_EXPONENT) or else
               Exponent.Integer <= MAX_FLOAT_EXPONENT - Digit_Array.Length(Partial_Number.Whole)))) then
               declare
                  Combined : Number(Float);
                  Whole : Number(Float);
               begin
                  if Partial_Number.Has_Fraction then
                     for Index in reverse Digit_Array.First_Index(Partial_Number.Fraction) .. Digit_Array.Last_Index(Partial_Number.Fraction) loop
                  ...
                  for Index in Digit_Array.First_Index(Partial_Number.Whole) .. Digit_Array.Last_Index(Partial_Number.Whole) - 1 loop
                     Whole.Float := Whole.Float + Standard.Float(Digit_Array.Element(Partial_Number.Whole, Index));
                     Whole.Float := Whole.Float * Standard.Float(BASE_10);
                  end loop;
                  Whole.Float := Whole.Float + Standard.Float(Digit_Array.Element(Partial_Number.Whole, Digit_Array.Last_Index(Partial_Number.Whole)));

                  Combined.Float := Combined.Float + Whole.Float;

                  if Partial_Number.Has_Exponent then
                     Combined.Float := Combined.Float * (Standard.Float(BASE_10) ** Exponent.Integer);
                  end if;
              ...
              and then (Exponent.Kind = NaN or else (Exponent.Kind = Integer and then
                                                         ((Partial_Number.Exponent_Is_Negative and then Exponent.Integer > MIN_LONG_FLOAT_EXPONENT) or else
                                                          Exponent.Integer <= MAX_LONG_FLOAT_EXPONENT - Digit_Array.Length(Partial_Number.Whole)))) then
               declare
               ...
               begin
                  if Partial_Number.Has_Fraction then
                     for Index in reverse Digit_Array.First_Index(Partial_Number.Fraction) .. Digit_Array.Last_Index(Partial_Number.Fraction) loop
                        Combined.Long_Float := Combined.Long_Float + Standard.Long_Float(Digit_Array.Element(Partial_Number.Fraction, Index));
                        Combined.Long_Float := Combined.Long_Float / Standard.Long_Float(BASE_10);
                     end loop;
                  end if;

                  for Index in Digit_Array.First_Index(Partial_Number.Whole) .. Digit_Array.Last_Index(Partial_Number.Whole) - 1 loop
                     Whole.Long_Float := Whole.Long_Float + Standard.Long_Float(Digit_Array.Element(Partial_Number.Whole, Index));
                     Whole.Long_Float := Whole.Long_Float * Standard.Long_Float(BASE_10);
                  end loop;
                  Whole.Long_Float := Whole.Long_Float + Standard.Long_Float(Digit_Array.Element(Partial_Number.Whole, Digit_Array.Last_Index(Partial_Number.Whole)));

                  Combined.Long_Float := Combined.Long_Float + Whole.Long_Float;

                  if Partial_Number.Has_Exponent then
                     Combined.Long_Float := Combined.Long_Float * (Standard.Long_Float(BASE_10) ** Exponent.Integer);
                  end if;
              ...
              and then (Exponent.Kind = NaN or else (Exponent.Kind = Integer and then
                                                         ((Partial_Number.Exponent_Is_Negative and then Exponent.Integer > MIN_LONG_LONG_FLOAT_EXPONENT) or else
                                                          Exponent.Integer <= MAX_LONG_LONG_FLOAT_EXPONENT - Digit_Array.Length(Partial_Number.Whole)))) then
               declare
               ...
               begin
                  if Partial_Number.Has_Fraction then
                     for Index in reverse Digit_Array.First_Index(Partial_Number.Fraction) .. Digit_Array.Last_Index(Partial_Number.Fraction) loop
                        Combined.Long_Long_Float := Combined.Long_Long_Float + Standard.Long_Long_Float(Digit_Array.Element(Partial_Number.Fraction, Index));
                        Combined.Long_Long_Float := Combined.Long_Long_Float / Standard.Long_Long_Float(BASE_10);
                     end loop;
                  end if;

                  for Index in Digit_Array.First_Index(Partial_Number.Whole) .. Digit_Array.Last_Index(Partial_Number.Whole) - 1 loop
                     Whole.Long_Long_Float := Whole.Long_Long_Float + Standard.Long_Long_Float(Digit_Array.Element(Partial_Number.Whole, Index));
                     Whole.Long_Long_Float := Whole.Long_Long_Float * Standard.Long_Long_Float(BASE_10);
                  end loop;
                  Whole.Long_Long_Float := Whole.Long_Long_Float + Standard.Long_Long_Float(Digit_Array.Element(Partial_Number.Whole, Digit_Array.Last_Index(Partial_Number.Whole)));

                  Combined.Long_Long_Float := Combined.Long_Long_Float + Whole.Long_Long_Float;

                  if Partial_Number.Has_Exponent then
                     Combined.Long_Long_Float := Combined.Long_Long_Float * (Standard.Long_Long_Float(BASE_10) ** Exponent.Integer);
                  end if;
            ...
            elsif Exponent.Kind = Integer and then Digit_Array.Length(Partial_Number.Whole) > BIG_NUMBER_DIGIT_LIMIT - Exponent.Integer - Digit_Array.Length(Partial_Number.Fraction) then
               return Number'(Kind => NaN);
            else
               declare
               ...
               begin
                  if Partial_Number.Has_Fraction then
                     for Index in reverse Digit_Array.First_Index(Partial_Number.Fraction) .. Digit_Array.Last_Index(Partial_Number.Fraction) loop
                        Combined.Big_Real := Big_Numbers.Big_Reals."+"(Combined.Big_Real, Big_Numbers.Big_Reals.To_Real(Standard.Integer(Digit_Array.Element(Partial_Number.Fraction, Index))));
                        Combined.Big_Real := Big_Numbers.Big_Reals."/"(Combined.Big_Real, Big_Numbers.Big_Reals.To_Real(BASE_10));
                     end loop;
                  end if;

                  for Index in Digit_Array.First_Index(Partial_Number.Whole) .. Digit_Array.Last_Index(Partial_Number.Whole) - 1 loop
                     Whole.Big_Real := Big_Numbers.Big_Reals."+"(Whole.Big_Real, Big_Numbers.Big_Reals.To_Real(Standard.Integer(Digit_Array.Element(Partial_Number.Whole, Index))));
                     Whole.Big_Real := Big_Numbers.Big_Reals."*"(Whole.Big_Real, Big_Numbers.Big_Reals.To_Real(BASE_10));
                  end loop;
                  Whole.Big_Real := Big_Numbers.Big_Reals."+"(Whole.Big_Real, Big_Numbers.Big_Reals.To_Real(Standard.Integer(Digit_Array.Element(Partial_Number.Whole, Digit_Array.Last_Index(Partial_Number.Whole)))));

                  Combined.Big_Real := Big_Numbers.Big_Reals."+"(Combined.Big_Real, Whole.Big_Real);

                  if Partial_Number.Has_Exponent then
                     Combined.Big_Real := Big_Numbers.Big_Reals."*"(Combined.Big_Real, Big_Numbers.Big_Reals."**"(Big_Numbers.Big_Reals.To_Real(10), Exponent.Integer));
                  end if;

                  if Partial_Number.Whole_Is_Negative then
                     Combined.Big_Real := Big_Numbers.Big_Reals."*"(Combined.Big_Real, Big_Numbers.Big_Reals.To_Real(-1));
                  end if;
   ...
   function To_Integer (Partial_Integer : Digit_Array.Unbound_Array; Is_Negative : Boolean) return Number is
   begin
      if Digit_Array.Is_Empty(Partial_Integer) or else Digit_Array.Length(Partial_Integer) > BIG_NUMBER_DIGIT_LIMIT then
         return Number'(Kind => NaN);
      elsif Digit_Array.Length(Partial_Integer) * BASE_10_LENGTH_TO_BIT_SIZE < Standard.Integer'Size then
         declare
            Combined : Number(Integer);
         begin
            for Index in Digit_Array.First_Index(Partial_Integer) .. Digit_Array.Last_Index(Partial_Integer) - 1 loop
               Combined.Integer := Combined.Integer + Standard.Integer(Digit_Array.Element(Partial_Integer, Index));
               Combined.Integer := Combined.Integer * BASE_10;
            end loop;
            Combined.Integer := Combined.Integer + Standard.Integer(Digit_Array.Element(Partial_Integer, Digit_Array.Last_Index(Partial_Integer)));
            if Is_Negative then
         ...
         begin
            for Index in Digit_Array.First_Index(Partial_Integer) .. Digit_Array.Last_Index(Partial_Integer) - 1 loop
               Combined.Long_Integer := Combined.Long_Integer + Standard.Long_Integer(Digit_Array.Element(Partial_Integer, Index));
               Combined.Long_Integer := Combined.Long_Integer * Standard.Long_Integer(BASE_10);
            end loop;
            Combined.Long_Integer := Combined.Long_Integer + Standard.Long_Integer(Digit_Array.Element(Partial_Integer, Digit_Array.Last_Index(Partial_Integer)));
            if Is_Negative then
         ...
         begin
            for Index in Digit_Array.First_Index(Partial_Integer) .. Digit_Array.Last_Index(Partial_Integer) - 1 loop
               Combined.Long_Long_Integer := Combined.Long_Long_Integer + Standard.Long_Long_Integer(Digit_Array.Element(Partial_Integer, Index));
               Combined.Long_Long_Integer := Combined.Long_Long_Integer * Standard.Long_Long_Integer(BASE_10);
            end loop;
            Combined.Long_Long_Integer := Combined.Long_Long_Integer + Standard.Long_Long_Integer(Digit_Array.Element(Partial_Integer, Digit_Array.Last_Index(Partial_Integer)));
            if Is_Negative then
         ...
         begin
            for Index in Digit_Array.First_Index(Partial_Integer) .. Digit_Array.Last_Index(Partial_Integer) - 1 loop
               Combined.Big_Integer := Ada.Numerics.Big_Numbers.Big_Integers."+"(Combined.Big_Integer, Ada.Numerics.Big_Numbers.Big_Integers.To_Big_Integer(Standard.Integer(Digit_Array.Element(Partial_Integer, Index))));
            ...
            Combined.Big_Integer := Ada.Numerics.Big_Numbers.Big_Integers."+"(Combined.Big_Integer, Ada.Numerics.Big_Numbers.Big_Integers.To_Big_Integer(Standard.Integer(Digit_Array.Element(Partial_Integer, Digit_Array.Last_Index(Partial_Integer)))));
            if Is_Negative then
